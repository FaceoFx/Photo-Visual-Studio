...
4.  Combine these three sources of information into a single, cohesive, and masterful prompt. The prompt should be structured logically (describing the scene, the product, the lighting, the mood, the camera details) and include strong keywords for photorealism and detail. Be creative but grounded in the user's choices.`;
  ...
  async function generateProductStudioImage(finalPrompt: string, useReference: boolean) {
    ...
    try {
        ...
        const { base64: productImageBase64, mimeType: productImageMimeType } = await resizeImageFile(productImageFile);
        const productPart = { inlineData: { mimeType: productImageMimeType, data: productImageBase64 } };
        parts.push(productPart);

        if (useReference && referenceImageFile) {
            const { base64: referenceImageBase64, mimeType: referenceImageMimeType } = await resizeImageFile(referenceImageFile);
            const referencePart = { inlineData: { mimeType: referenceImageMimeType, data: referenceImageBase64 } };
...
**FINAL OUTPUT:** A single, cohesive, and believable image where the modifications and the integrated product are perfectly blended and appear as if they were captured in a single photograph.`;
        ...
        case 'keep_model_replace_object':
          directiveInstruction = `**NON-NEGOTIABLE DIRECTIVE: MASTER-LEVEL MODEL PRESERVATION & OBJECT REPLACEMENT.**

**ASSET DEFINITIONS (CRITICAL):**
- **FIRST IMAGE provided:** The **'New Product'**. This is the object that MUST be placed into the final scene.
- **SECOND IMAGE provided:** The **'Reference Scene'**. This image contains the scene, background, and most importantly, the human model. The model in this image is SACROSANCT.

**CORE MISSION:** Your absolute highest priority is to perform a surgical object replacement. You will take the **'New Product'** (from the FIRST image) and place it into the **'Reference Scene'** (from the SECOND image), while preserving the human model with perfect, unchangeable fidelity.

**ABSOLUTE RULE - MODEL IMMUTABILITY (FAILURE IS NOT AN OPTION):**
- You are **STRICTLY FORBIDDEN** from altering, re-rendering, redrawing, or modifying the human model(s) from the **'Reference Scene'** in any conceivable way.
- The model's pose, expression, anatomy, skin, hair, clothing, and the existing lighting *on the model* are **100% immutable and must be preserved with pixel-perfect accuracy**.
- Treat the human model as a locked, un-editable foreground layer that must appear in the final image exactly as it does in the **'Reference Scene'**.

**EXECUTION WORKFLOW:**
1.  **IDENTIFY & EXCISE TARGET OBJECT:** Analyze the **'Reference Scene'**. Identify the primary object the model is holding, wearing, or interacting with (e.g., a handbag, a coffee cup, a phone). Perform a surgical, pixel-perfect removal of this object. The area behind the removed object (including parts of the model's hands or clothing that were obscured) must be flawlessly reconstructed using generative fill that matches the background and surrounding textures perfectly.

2.  **ISOLATE & PREPARE NEW PRODUCT:** Isolate the main subject from the **'New Product'** image with a clean, perfect mask.

3.  **SEAMLESS COMPOSITE & INTERACTION (CRITICAL):** Place the isolated **'New Product'** into the prepared **'Reference Scene'** where the original object was.
    - **Scale & Perspective:** The scale, perspective, and angle of the new product MUST be 100% physically plausible and match the original scene's camera properties.
    - **Hand Interaction:** If the original object was held, the **'New Product'** must be placed so that the model's hands are interacting with it in a completely natural and convincing way. The hands MUST wrap around or hold the new object realistically. You may need to subtly reconstruct fingers that were previously holding the old object to fit the new one.

4.  **LOCALIZED RE-RENDERING (THE SECRET TO REALISM):** This is the final and most critical step. You must re-render the lighting, shadows, and reflections **ONLY ON THE NEWLY PLACED PRODUCT AND THE IMMEDIATE INTERACTION POINTS (e.g., fingers touching it)**.
    - The new product must perfectly inherit the lighting environment from the **'Reference Scene'**.
    - It must cast realistic contact shadows and cast shadows onto the model's body, hands, and the background.
    - It must receive color bleed and reflections from the model and the environment. For example, if the model is wearing a red shirt, the new product should have a subtle red reflection on its surface.
    - **CRUCIAL:** The lighting on the rest of the model and the background themselves MUST NOT be changed. You are only lighting the new object to match the existing, locked-in lighting of the scene.

**FINAL CHECK:** The final output must look like a single, original photograph where the only change is the object being held/used. Any modification to the human model's face, body, or pose is a complete failure.`;
          break;
        case 'none':
        default:
          break;
